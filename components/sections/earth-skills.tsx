...
function OrbitingSkills({
  ...
  const positions = useMemo(() => {
    ...
    skills.forEach((_, i) => (i % 2 === 0 ? ringAIdx.push(i) : ringBIdx.push(i)))

    const pos: [number, number, number][] = new Array(skills.length).fill([0, 0, 0])
    ...
    ringAIdx.forEach((i, idx) => (pos[i] = toPos(idx, Math.max(1, ringAIdx.length), 20)))
    ringBIdx.forEach((i, idx) => (pos[i] = toPos(idx, Math.max(1, ringBIdx.length), -25)))
    return pos
  }, [skills])
    ...
    <group ref={group}>
      {skills.map((s, i) => (
        <SkillMarker
          ...
          label={s.label}
          color={i % 3 === 0 ? ACCENT_ORANGE : i % 3 === 1 ? PRIMARY_BLUE : NEUTRAL_GRAY}
          href={s.href}
        />
      ))}
    </group>
    ...
    <section className="relative mx-auto w-full max-w-6xl overflow-hidden rounded-xl border bg-card p-4 md:p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-pretty text-2xl font-semibold leading-tight md:text-3xl">
            SACHIN BURNWAL — Skills in Orbit
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            A colorful, interactive 3D visualization of my core skills rotating around Earth.
          </p>
        </div>
      </div>
      ...
      <div className="relative h-[460px] w-full rounded-lg">
        ...
        >
          ...
          <directionalLight position={[4, 3, 5]} intensity={1.1} color={NEUTRAL_GRAY} />
          <directionalLight position={[-4, -1, -3]} intensity={0.35} color={PRIMARY_BLUE} />
          ...
          {/* Camera controls */}
          <OrbitControls enableZoom={false} enablePan={false} autoRotate={!reducedMotion} autoRotateSpeed={0.4} />
        </Canvas>
      </div>
